		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWFirewallGroupV2("telefonicaopencloud_fw_firewall_group_v2.fw_1", "", "", ipolicyID, epolicyID),
				),
			},
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWFirewallGroupV2(
						"telefonicaopencloud_fw_firewall_group_v2.fw_1", "fw_1", "terraform acceptance test", ipolicyID, epolicyID),
				),
			},
		},
	})
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWFirewallGroupV2Exists("telefonicaopencloud_fw_firewall_group_v2.fw_1", &firewall_group),
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWFirewallGroupV2Exists("telefonicaopencloud_fw_firewall_group_v2.fw_1", &firewall_group),
					resource.TestCheckResourceAttr("telefonicaopencloud_fw_firewall_group_v2.fw_1", "description", "firewall router test"),
...
func testAccCheckFWFirewallGroupV2Destroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefonicaopencloud_firewall_group" {
			continue
		}
...
func testAccCheckFWFirewallGroupV2Exists(n string, firewall_group *FirewallGroup) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		networkingClient, err := config.networkingV2Client(OS_REGION_NAME)
		if err != nil {
			return fmt.Errorf("Exists) Error creating TelefonicaOpenCloud networking client: %s", err)
		}
...
resource "telefonicaopencloud_fw_firewall_group_v2" "fw_1" {
  ingress_policy_id = "${telefonicaopencloud_fw_policy_v2.policy_1.id}"
  egress_policy_id = "${telefonicaopencloud_fw_policy_v2.policy_1.id}"
...
resource "telefonicaopencloud_fw_firewall_group_v2" "fw_1" {
  ...
  ingress_policy_id = "${telefonicaopencloud_fw_policy_v2.policy_2.id}"
  egress_policy_id = "${telefonicaopencloud_fw_policy_v2.policy_2.id}"
  admin_state_up = true
...
resource "telefonicaopencloud_fw_policy_v2" "policy_2" {
  name = "policy_2"
}
`
...
resource "telefonicaopencloud_fw_firewall_group_v2" "fw_1" {
  ...
  #egress_policy_id = "${telefonicaopencloud_fw_policy_v2.policy_1.id}"
  ports = [
	"${telefonicaopencloud_networking_port_v2.port_1.id}"
  ]
  depends_on = ["telefonicaopencloud_networking_router_interface_v2.router_interface_1"]
}
`, OS_EXTGW_ID)
...
resource "telefonicaopencloud_networking_router_interface_v2" "router_interface_2" {
  router_id = "${telefonicaopencloud_networking_router_v2.router_2.id}"
...
resource "telefonicaopencloud_fw_firewall_group_v2" "fw_1" {
  ...
  ports = [
	"${telefonicaopencloud_networking_port_v2.port_1.id}",
	"${telefonicaopencloud_networking_port_v2.port_2.id}"
  ]
  depends_on = ["telefonicaopencloud_networking_router_interface_v2.router_interface_1", "telefonicaopencloud_networking_router_interface_v2.router_interface_2"]
}
