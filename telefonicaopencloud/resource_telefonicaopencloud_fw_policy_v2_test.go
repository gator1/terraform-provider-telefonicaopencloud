			...
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWPolicyV2Exists(
						"telefonicaopencloud_fw_policy_v2.policy_1", "", "", 0),
				),
			},
		},
	})
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWPolicyV2Exists(
						"telefonicaopencloud_fw_policy_v2.policy_1", "policy_1", "terraform acceptance test", 2),
				),
			},
		},
	})
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWPolicyV2Exists(
						"telefonicaopencloud_fw_policy_v2.policy_1", "policy_1", "terraform acceptance test", 1),
				),
			},
		},
	})
...
func testAccCheckFWPolicyV2Destroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefonicaopencloud_fw_policy_v2" {
			continue
		}
...
resource "telefonicaopencloud_fw_policy_v2" "policy_1" {
  ...
  rules = [
    "${telefonicaopencloud_fw_rule_v2.udp_deny.id}",
    "${telefonicaopencloud_fw_rule_v2.tcp_allow.id}"
  ]
}

resource "telefonicaopencloud_fw_rule_v2" "tcp_allow" {
  protocol = "tcp"
  action = "allow"
}

resource "telefonicaopencloud_fw_rule_v2" "udp_deny" {
  protocol = "udp"
  action = "deny"
}
`
